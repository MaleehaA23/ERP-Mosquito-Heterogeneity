...
// Model 4:hierarchical Bayesian model which expands Model 2 by varying biting rate by volunteer.
data {
  ...
  int<lower=1> Nvolunteers;
  int<lower=1, upper=Nvolunteers> Volunteer0[n0];
  int<lower=1, upper=Nvolunteers> Volunteer1[n1];
}
...
parameters {
  ...
  //volunteer random effect on biting
  real<lower=0> sigma_vol;  
  vector[Nvolunteers] z_vol;
}

transformed parameters {
  // control
  vector[n0] log_totalC; // log rate of total caught in control (fed + UD)
  vector[n0] log_qC;     // log P(fed | caught) in control
  ...
  for(j in 1:n1) {
    real log_alphaB_j =
      ...
      sigma_vol * z_vol[Volunteer1[j]] +  // Add volunteer-level heterogeneity to biting
      log(samples1[j]);
...
model {
  ...
  beta ~ normal(-1, 1);
  kappa ~ normal(-1, 1);
  sigma_vol ~ cauchy(0, hierarchy);   //volunteer
  z_vol ~ normal(0, 1);
...
generated quantities {
  ...
  vector[Nvolunteers] volunteer_effect_out;
  for(d in 1:Ndays){
  alpha_day_out[d] =
 ...
 for (v in 1:Nvolunteers) {
    volunteer_effect_out[v] =
    exp(sigma_vol * z_vol[v]);
    }
