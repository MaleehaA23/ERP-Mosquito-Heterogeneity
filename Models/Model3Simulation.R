...
## Posterior samples
alphaB_post <- read.csv("ArabiensisNewResults/Model3/Post_alphaB.csv")[,1]
alphaM_post <- read.csv("ArabiensisNewResults/Model3/Post_alphaM.csv")[,1]

sigma_alpha_post  <- read.csv("ArabiensisNewResults/Model3/Post_sigma_alpha.csv")[,1]
sigma_alpha2_post <- read.csv("ArabiensisNewResults/Model3/Post_sigma_alpha2.csv")[,1]

alpha_day_post  <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_alpha_day.csv"))
alpha2_day_post <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_alpha2_day.csv"))

beta_post  <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_beta.csv"))
kappa_post <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_kappa.csv"))

sigma_beta_post  <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_sigma_beta.csv"))
sigma_kappa_post <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_sigma_kappa.csv"))

beta_day_post  <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_beta_day.csv"))
kappa_day_post <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_kappa_day.csv"))

phiC <- read.csv("ArabiensisNewResults/Model3/Post_phiC.csv")[,1]
phiI <- as.matrix(read.csv("ArabiensisNewResults/Model3/Post_phiI.csv"))
...
lower_sigma_beta  <- upper_sigma_beta  <- matrix(NA, nsim, Nnets)
lower_sigma_kappa <- upper_sigma_kappa <- matrix(NA, nsim, Nnets)
...
true_sigma_beta <- matrix(NA, nsim, Nnets)
est_sigma_beta  <- matrix(NA, nsim, Nnets)

true_sigma_kappa <- matrix(NA, nsim, Nnets)
est_sigma_kappa  <- matrix(NA, nsim, Nnets)
...
for(sim in 1:nsim){
  ...
  sigma_beta <- sigma_beta_post[draw, ]
  sigma_kappa <- sigma_kappa_post[draw, ]
  
  alpha_day  <- alpha_day_post[draw, ]
  alpha2_day <- alpha2_day_post[draw, ]
  
  beta_day <- matrix(
    as.numeric(beta_day_post[draw, ]),
    nrow = Nnets,
    ncol = Ndays,
    byrow = FALSE
  )
  ...
  kappa_day <- matrix(
    as.numeric(kappa_day_post[draw, ]),
    nrow = Nnets,
    ncol = Ndays,
    byrow = FALSE
  )
  ...
  for(i in 1:nrow(Data1_sim)){
    ...
    beta_net_day <- beta_day[net, day]
    kappa_net_day <- kappa_day[net, day]
    
    feed <- alphaB_day * beta_net_day
    kill <- alphaB_day * kappa_net_day
    natural <- alphaM_day
  ...
  true_sigma_beta[sim, ] <- sigma_beta
  true_sigma_kappa[sim, ] <- sigma_kappa
  ...
  sigma_beta_out <- posterior[, grep("^sigma_beta\\[", colnames(posterior))]
  sigma_kappa_out <- posterior[, grep("^sigma_kappa\\[", colnames(posterior))]
  ...
  est_sigma_beta[sim, ] <- apply(sigma_beta_out, 2, mean)
  est_sigma_kappa[sim, ] <- apply(sigma_kappa_out, 2, mean)
  ...
  lower_sigma_beta[sim, ] <- apply(
    sigma_beta_out, 2, quantile, probs = 0.025
  )
  upper_sigma_beta[sim, ] <- apply(
    sigma_beta_out, 2, quantile, probs = 0.975
  )
  
  lower_sigma_kappa[sim, ] <- apply(
    sigma_kappa_out, 2, quantile, probs = 0.025
  )
  upper_sigma_kappa[sim, ] <- apply(
    sigma_kappa_out, 2, quantile, probs = 0.975
  )
...
cat(
  "sigma_alpha:",
  cor(true_sigma_alpha, est_sigma_alpha, use = "complete.obs"),
  "\n"
)

cat(
  "sigma_alpha2:",
  cor(true_sigma_alpha2, est_sigma_alpha2, use = "complete.obs"),
  "\n\n"
)
...
for(i in 1:Nnets){
  cat(
    "sigma_beta", i, ":",
    cor(true_sigma_beta[, i], est_sigma_beta[, i], use = "complete.obs"),
    "\n"
  )
}
...
for(i in 1:Nnets){
  cat(
    "sigma_kappa", i, ":",
    cor(true_sigma_kappa[, i], est_sigma_kappa[, i], use = "complete.obs"),
    "\n"
  )
}
...
results <- list(
  ...
  est_sigma_beta = est_sigma_beta,
  lower_sigma_beta = lower_sigma_beta,
  upper_sigma_beta = upper_sigma_beta,
  
  true_sigma_kappa = true_sigma_kappa,
  est_sigma_kappa = est_sigma_kappa,
  lower_sigma_kappa = lower_sigma_kappa,
  upper_sigma_kappa = upper_sigma_kappa
)
...
saveRDS(
  results,
  "SimulationResults/Model3ParameterRecoveryResults.rds"
)